...
/// <summary>
/// SekaiPlatform 后端 HTTP 客户端。原 <c>SekaiToolsMauiText.Services.SekaiPlatformClient</c>
/// 平移过来，把 MAUI 的 <c>Microsoft.Maui.Storage.Preferences</c> 替换成可注入的
/// <see cref="IPlatformPreferences"/>，桌面端用 JSON 文件实现，移动端可直接套 MAUI Preferences。
///
/// 行为：
/// - base url / access token 持久化在 preferences 里
/// - 登录 / 切租户 / 拿会话成功后会自动写回 access token，并设置 Authorization header
/// - 收到 401 自动清空 token
/// </summary>
public sealed class SekaiPlatformClient
{
    ...
    private readonly IPlatformPreferences _preferences;
    ...
    public SekaiPlatformClient(IPlatformPreferences preferences)
    {
    ...
    {
        return _preferences.Get(BaseUrlKey, DefaultBaseUrl).TrimEnd('/');
    }
    ...
    public void SetBaseUrl(string baseUrl)
    {
        ...
        _preferences.Set(BaseUrlKey, normalized);
    ...
    public Task<IReadOnlyList<PlatformStoryGroup>> GetStoryGroupsAsync(
    ...
    public Task<IReadOnlyList<PlatformStory>> GetStoriesAsync(
    ...
    public Task<IReadOnlyList<PlatformSourceLine>> GetStorySourceLinesAsync(
    ...
    public Task<IReadOnlyList<PlatformTranslationVersion>> GetTranslationVersionsAsync(
        long storyId, CancellationToken cancellationToken = default)
    {
    ...
    public Task<IReadOnlyList<PlatformTranslationLine>> GetTranslationLinesAsync(
    ...
    {
        var token = _preferences.Get(AccessTokenKey, string.Empty);
    ...
    private void SaveToken(string? token)
    {
        ...
        _preferences.Set(AccessTokenKey, token);
        _httpClient.DefaultRequestHeaders.Authorization =
            new System.Net.Http.Headers.AuthenticationHeaderValue("Bearer", token);
    }
